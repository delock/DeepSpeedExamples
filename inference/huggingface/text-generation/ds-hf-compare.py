...
for prompt, base_out in zip(inputs, base_out_list):
    ...
    if args.local_rank == 0:
        if args.print_outputs:
            print_0(f"baseline output: {base_out}")
            print_0(f"deepspeed output: {ds_out}")
            print_0(f"{'-'*60}")
        if base_out == ds_out[0]:
            if args.print_outputs: print_0("outputs match")
            match_count += 1
        else:
            if args.print_outputs: print_0("outputs do not match")
            mismatch_count += 1
        similarity = string_similarity(base_out['generated_text'], ds_out[0]['generated_text'])
        if args.print_outputs: print_0(f"The similarity ratio is: {similarity*100}%")
